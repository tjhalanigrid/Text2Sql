...
ADAPTER_PATH = os.path.join(PROJECT_ROOT, "checkpoints/sft_adapter")
...
class SQLVocabularyLogitsProcessor(LogitsProcessor):
    """
    Hard mask logits to allow only a schema-aware SQL vocabulary.

    IMPORTANT: the model's output vocab size can differ from `len(tokenizer)` if
    embeddings were resized. We therefore build the bias mask dynamically from
    `scores.shape[-1]` at runtime to avoid shape mismatches.
    """
...
#     learning_rate=1e-6,            # was 5e-6 (too high → forgetting SQL)
#     batch_size=2,                  # reduce gradient noise
#     mini_batch_size=1,
#     gradient_accumulation_steps=2,
#     ppo_epochs=1,                  # critical: avoid over-updating

#     init_kl_coef=2.5,              # STRONG KL to stop collapse
#     adap_kl_ctrl=True,
#     target_kl=0.005,               # tighter leash

#     cliprange=0.02,                # smaller updates
#     cliprange_value=0.05,
#     whiten_rewards=False,          # IMPORTANT → was amplifying noise
#     kl_penalty="kl",               # was "full" (too aggressive)
# )
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    # save checkpoint
    epoch_out = os.path.join(PROJECT_ROOT, f"outputs/rlhf_text2sql_epoch_{epoch}")
    os.makedirs(epoch_out, exist_ok=True)
    trainer.model.save_pretrained(epoch_out)
    tokenizer.save_pretrained(epoch_out)
